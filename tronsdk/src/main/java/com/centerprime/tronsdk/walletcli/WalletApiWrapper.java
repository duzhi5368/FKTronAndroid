...
public class WalletApiWrapper {
  ...
  public String importWallet(char[] password, String priKey) throws CipherException, IOException {
    //byte[] privateKey2 = StringUtils.hexStringToByteArray(priKey);
    byte[] privateKey3 = StringUtils.parseHexBinary(priKey);
    //byte[] privateKey = priKey.getBytes();
    if (!WalletApi.passwordValid(password)) {
      return null;
    }
    if (!WalletApi.priKeyValid(privateKey3)) {
      return null;
    }
    byte[] passwd = StringUtils.char2Byte(password);
    wallet = new WalletApi(passwd, privateKey3);
    wallet.setLogin();
    StringUtils.clear(passwd);
